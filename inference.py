...
# Так как подсчет символов влияет на инференс, а самое длинное слово в русском 55 букв
# я просто захардкодил, но можно убрать комментарии со следующих 2 строк кода, где идет подсчет
# но такие значения лучше кэшировать где-то, чтобы не пересчитывать каждый раз
MAX_WORD_LENGTH = 55
...
def primary_segment(text, word_probs):
    ...
    # а если такого нет, закидываем просто обычный text
    return words[n] if words[n] else [text]
...
# тут мега крутая функция, принимает на вход текст без пробелов, но с спец символами
# и текст с пробелами но без спец символов, возвращает текст с пробелами и спец символами
